...
class UserService
{
    ...
    {
        $subject = 'Confirm your account for ' . Application::getConfig()->get('SITE_NAME', 'blueprintUE');
        $html = static::getConfirmAccountEmailHTML($token, $username);
        $text = 'Welcome to ' . Application::getConfig()->get('SITE_NAME', 'blueprintUE') . "\n\n";
    ...
    protected static function getConfirmAccountEmailHTML(string $token, string $username): string
    {
        ...
        $replace = [
            ...
            $url,
            $now->format('Y'),
            Security::escHTML(Application::getConfig()->get('SITE_NAME', 'blueprintUE')),
            Security::escAttr(Application::getConfig()->get('SITE_NAME', 'blueprintUE')),
            Security::escHTML($username),
            Security::escAttr(Helper::getHostname() . '/' . Security::escAttr(Application::getConfig()->get('MAIL_HEADER_LOGO_PATH', 'blueprintue_full-logo.png'))), // phpcs:ignore
        ];
